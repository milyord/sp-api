...
def _get_kwargs(
    *,
    ...
    url = "{}/vendor/directFulfillment/orders/2021-12-28/acknowledgements".format(client.base_url)
...
def _parse_response(*, client: Client, response: httpx.Response) -> Optional[Union[ErrorList, TransactionId]]:
    if response.status_code == HTTPStatus.ACCEPTED:
        response_202 = TransactionId.from_dict(response.json())
...
def _build_response(*, client: Client, response: httpx.Response) -> Response[Union[ErrorList, TransactionId]]:
    return Response(
...
def sync_detailed(
    *,
    client: Client,
    json_body: SubmitAcknowledgementRequest,
) -> Response[Union[ErrorList, TransactionId]]:
...
def sync(
    *,
    client: Client,
    json_body: SubmitAcknowledgementRequest,
) -> Optional[Union[ErrorList, TransactionId]]:
    """Submits acknowledgements for one or more purchase orders.
